...
function render() {
    ...
    for (let i = 0; ; i++) {
        if (window.ponds[i + seed + searches][goggles].match(/^Legendary/)) {
            document.querySelector('#nextleg').textContent = `Your next Legendary is ${window.ponds[i + seed + searches][goggles].replace(/^Legendary:/g, '')} in ${i + 1} searches`;
            break;
        }
    }

    document.querySelector('#result').innerHTML = window.ponds.slice(seed + searches, 1000 + seed + searches).map((v, k) => `<span class="${v[goggles].replace(':', ' ')}">${k + 1} (seed #${seed + searches + k}) - ${v[goggles].replace(':', ' ')}</span>`).join("");
}